...
status = ['_help','Trying to play music','Chillin', 'Searching wikipedia', 'Created By SAKEC']
...
async def meme(ctx):
    sbrdmemes = ("dankmemes", "memes", "funny")
...
async def joke(ctx):
    sbrdjokes = ("dadjokes", "unclejokes", "jokes")
    subreddit= reddit.subreddit(random.choice(sbrdjokes))
    all_subs=[]
...
@client.command(name='holup', aliases = ["holdup", "hp"], help='Holup Memes')
async def holup(ctx):
...
client.run("ODkxOTc2MDg0MzgyNDI5MTk1.YVGLdg.WayA_ILsCoTsdZekJLAt5htgrrE")